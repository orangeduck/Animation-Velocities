    ...
    quat next, quat curr, float dt, float eps=1e-8f)
{
    return quat_to_scaled_angle_axis(
        quat_abs(quat_mul_inv(next, curr)), eps) / dt; 
}
...
vec3 quat_delta_to_angular_velocity_alt(quat qref, quat qdelta)
{
    const float delta = 0.001f;
    quat q2 = quat_normalize(qref + delta * qdelta);
    return quat_to_scaled_angle_axis(quat_abs(quat_mul_inv(q2, qref))) / delta;
}

static inline quat quat_normalize_dx(quat q, quat dq, const float eps = 1e-8f)
{
    return (dq - quat_normalize(q) * quat_dot(dq, quat_normalize(q))) / 
        (quat_length(q) + eps);
}

static inline quat quat_between_dx(vec3 p, vec3 dp, vec3 q, vec3 dq)
{
    vec3 c = cross(p, q);
